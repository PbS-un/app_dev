...
class e extends WorkerTaskImpl{constructor(){super(),console.log("[WorkerTask] HelloWorkerTask 构造器初始化")}entry(){console.log("[WorkerTask] HelloWorkerTask 启动完成，等待主线程消息")}onMessage(e=null){console.log("[WorkerTask] 收到主线程消息: ",UTS.JSON.stringify(e)),this.sendReply(e)}sendReply(e=null){console.log("[WorkerTask] 发送回复消息"),this.postMessage(e)}}(new e).entry();
