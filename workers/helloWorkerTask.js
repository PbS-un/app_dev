...
class e extends WorkerTaskImpl{constructor(){super(),console.log("[WorkerTask] HelloWorkerTask 构造器初始化")}entry(){console.log("[WorkerTask] HelloWorkerTask 启动完成，等待主线程消息")}onMessage(e=null){console.log("[WorkerTask] 收到主线程消息: ",e);const o=e,s=o.data,n=o.needReply;console.log("[WorkerTask] 数据: ".concat(s,", 需要回复: ").concat(n)),n&&this.sendReply(s)}sendReply(e){const o=new UTSJSONObject,s=parseInt(e);if(isNaN(s))o.data='输入"'.concat(e,'"不是有效数字');else{const e=s+1;o.data=e.toString()}console.log("[WorkerTask] 发送回复消息:",o),this.postMessage(o)}}(new e).entry();
